...
const BetsApiSlice = apiSlice.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    sportsMenu: builder.query<SportsMenuResponse, SportMenuDto>({
      query: (data) => ({
    ...
    fetchFixtures: builder.mutation<FetchFixtureResponse, FixturesDto>({
      query: (data) => ({
    ...
    fetchBetHistory: builder.query<GetBetListResponse, { page: number }>({
      query: (data) => ({
    ...
    >({
      ...
      onQueryStarted: async (_, { dispatch, queryFulfilled }) => {
        try {
          ...
          dispatch(setLiveFixtures(liveFixtures));
        } catch (error) {
          console.error("Error processing live fixtures:", error);
        }
      },
    }),
    topBets: builder.query<TopBetsResponse, void>({
      query: () => ({
